...
describe("Home Page", () => {
  ...
  it("displays project number in header", () => {
    ...
    const headerProject = screen.getByText("Project 38222", {
      selector: "span",
    });
  ...
  it("renders navigation links in header", () => {
    ...
    const navLinks = ["DataSets", "Import", "Jobs", "gStore", "Help"];

    navLinks.forEach((link) => {
      // Use getAllByText and check that at least one is a link in nav
      const linkElements = screen.getAllByText(link);
      const navLink = linkElements.find(
        (element) => element.tagName === "A" && element.closest("nav"),
      );
  ...
  it("renders menu cards with correct descriptions", () => {
    ...
    expect(
      screen.getByText(/You can see, edit and delete DataSets/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Import a DataSet from .tsv file/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Check your submitted jobs and the status/),
    ).toBeInTheDocument();
